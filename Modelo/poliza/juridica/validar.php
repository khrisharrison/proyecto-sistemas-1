...
$sql = "SELECT * FROM RESPONSABLE_JURIDICO WHERE RIF_juridico = '$cedula'";
...
if ($result->num_rows > 0) {
	// La cédula ya existe en la base de datos
	$response = array("status" => "error", "message" => "el responsable de esta poliza ya esta registrado como persona natural");
	echo json_encode($response);
	exit();
}
