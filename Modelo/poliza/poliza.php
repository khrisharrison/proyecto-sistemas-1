...
<tbody>
    <?php
$consulta_pagina_actual = "SELECT * FROM poliza";
...
while($row = mysqli_fetch_array($resultado_pagina_actual)) {?>
    <tr>
        ...
        <td>
            <a href="edit.php?id=<?php echo $row['NUMERO_POLIZA']?>" type="button" onclick="return ConfirmarEditar()">
                <i class="btn btn-primary btn-sm">Modificar</i><!--cambiar por codigo-->
            </a>
            <a href="delete.php?id=<?php echo $row['NUMERO_POLIZA']?>" type="button" onclick="return ConfirmarEliminar()">
